...
- 原文件上传（教案库 / 周计划库 / 成果库）：先 `POST /api/file/upload`（multipart `file`），再尝试 `POST /api/knowledge/document/file`（JSON `file_id`/`file_url`）或回退 `document/text` 写入可检索说明与原文件链接；分类由 `forceKind` 指定，禁止教案/周计划误入教师成果库
