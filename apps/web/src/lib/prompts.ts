...
【任务】
根据用户提供的成果材料（文件名、文件地址、可提取正文、平台 OCR 文本等），解析出可供成果库展示、本地入库与「专业成长树」挂果的结构化内容。
你要做的是「看懂材料在说什么」，而不是复述文件格式或空泛介绍截图软件。
成长树果实只有三类，必须准确归入其一，并尽量抽出成果年份。

【成长树分类 treeCategory（必选其一，用于挂果）】
1. 特色实践：班本/园本主题活动、项目课程、环境创设、区域游戏、家园共育、生活活动、观察记录、家园沟通截图、教学过程资料、实践反思。
2. 教研科研：公开课/研讨课、论文、课题、课例、个案研究、教研发言、专业案例写作。
3. 专业荣誉：奖状证书、表彰决定、竞赛获奖、骨干认定、研修培训证明、学历/资格证书、会议参会证明。

分类决策（按顺序，命中即停）：
A. 出现奖项名称、等次、颁奖单位、证书编号、荣誉称号 → 专业荣誉。
B. 出现论文/课题/公开课/个案/课例/教研组研讨等研究表达 → 教研科研。
C. 其余真实保教实践材料 → 特色实践。
D. 信息不足无法判断：treeCategory 仍填「特色实践」，needsHumanReview=true，并在 summary 写明「分类待核对」。

【材料细类 materialType（在 treeCategory 之下再选一，便于摘要）】
特色实践：主题活动 / 项目课程 / 环境创设 / 家园共育 / 观察记录 / 教学过程资料 / 其他实践
教研科研：公开课 / 论文案例 / 课题研究 / 幼儿个案 / 教研发言 / 其他教研
专业荣誉：竞赛获奖 / 荣誉表彰 / 研修培训 / 资格证书 / 其他荣誉
不要再使用旧标签「专业研究成果 / 获奖与荣誉 / 学习与研修 / 其他」作为 treeCategory。

【年份 year】
- 优先取材料中的公历年份（证书落款、活动日期、论文发表年、培训起止年）。
- 只输出 1990～当前年+1 的四位整数。
- 同年份多种日期取最能代表该成果的年份（获奖取颁奖年，论文取发表年）。
- 完全看不到年份：year 填 0，needsHumanReview=true，不得编造。
...
3. 若仅有文件名与链接、无可读正文：summary/body 须说明「缺少可解析正文，需打开原文件人工核对」，needsHumanReview=true；treeCategory 按文件名关键词尽量归类，否则用「特色实践」；year=0；不得编造获奖名次、课题名称、论文结论等事实。
...
【输出 JSON 字段】
{
  ...
  "summary": "80～200 字成果摘要，写清材料主题、关键事实与年份（若有）",
  "body": "结构化正文（可用换行与小标题），包含：材料说明、识别到的要点、对保教工作的意义（若可判断）",
  "treeCategory": "特色实践 | 教研科研 | 专业荣誉",
  "materialType": "细类名称，见上文",
  "year": 2026,
  "keyPoints": ["要点1", "要点2"],
...
export function buildArchiveParseUserMessage(params: {
  ...
  const parts = [
    '请解析下列教师成果材料，并按系统要求只输出 JSON。必须给出 treeCategory（特色实践/教研科研/专业荣誉）和 year（四位年份，未知填 0）。',
    `文件名：${params.fileName}`,
