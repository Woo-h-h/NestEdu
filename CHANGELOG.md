...
- 业务：老师可在右上角、侧栏和首页打开「使用说明」，内容与操作手册一致。
- 技术：路由 `/help`；说明正文在 `lib/teacherHandbook.ts`。
- 验证：`pnpm --filter ./apps/web build`；打开 `/help`，并从首页、顶栏进入。

## 2026-09-02 — 教师操作手册

- 业务：新增给一线老师看的操作说明（登录、五入口、保存与按年查看、常见问题），不写后台术语。
- 技术：文档见 `docs/teacher-handbook.md`。
- 验证：对照侧栏首页 / 活动方案 / 周计划 / 成果库 / 教师画像现行按钮与提示通读一遍。


## 2026-08-25 — 智能画像份数与成长数据对齐

- 业务：解读区不再把活动方案/周计划显示成 0；改为展示当前入库与成果库份数，与上方成长树同源。
- 技术：快照表增加 `activity_plan_count` / `weekly_plan_count`；生成时成果库文档同样剔除误入的方案/周计划。
- 验证：刷新 `/profile`，「当前材料」活动方案/周计划应与入库记录一致；`cd apps/api && go test ./internal/service/ -run ProfileSnapshot`。

## 2026-08-25 — 成长树三类果实改配色

- 业务：特色实践为绿色果实，教研科研为橙色，专业荣誉为红色；结构卡与树上果实一致。颜色仅作类型区分。
- 技术：更新 `GROWTH_TREE_BRANCHES` 与 SVG 果实渐变。
- 验证：刷新 `/profile`，三张结构卡与树上果实应为绿 / 橙 / 红。

## 2026-08-25 — 修复画像页偶发缺少 X-Uid-Hash

- 业务：打开教师画像不再因为登录标识尚未就绪就弹出「画像保存异常」；正文已生成时以提示代替误报。
- 技术：uid_hash 写入 sessionStorage 并与当前 token 绑定；拉取/保存快照前先 `ensureUidHashForBff`，失败再拉一次 `/user/self`；刷新手机号缓存时不再清空该头。
- 验证：`pnpm --filter ./apps/web build`；登录后反复进出 `/profile`，黄条不应再无故出现。

## 2026-08-25 — 更新 README 对齐成长树与年份口径

- 业务：说明画像按年统计、成果库三类筛选、日常教学 `year` 与解析智能体 14509。
- 技术：同步 README 功能表、知识库/Agent、MySQL 表与 FAQ。
- 验证：通读 README「功能一览 / 教师画像 / 本地数据」与当前 `/profile`、`/archive` 行为一致。

## 2026-08-25 — 成长结构卡片按所选年份统计

- 业务：下方四张成长结构卡只显示当前选中年的叶片与果实，不再把成果库累计放到 2026。证书标题中的年份优先于上传年份。
- 技术：果实 `year` 优先取标题四位年；结构卡继续只用 `yearItems`。
- 验证：`pnpm --filter ./apps/web build`；选 2026 时专业荣誉不应再包含 2018 年证书；点 2018 才应看到那 2 份。

## 2026-08-25 — 画像按年统计叶片与果实

- 业务：切换成长年度后，「本年度产出 = 日常叶片 + 成果果实」；树冠底色不计入叶片。查看 2020 时不再把其他年份的果实算进当年。
- 技术：年份视图对日常与成果库文档统一按 `year` 过滤。
- 验证：`pnpm --filter ./apps/web build`；打开 `/profile` 选 2020，左侧三数相加应一致。

## 2026-08-25 — 日常教学入库增加年份字段

- 业务：活动方案 / 周计划写入 `teacher_generated_docs.year`，教师画像「日常教学」按该年份挂叶片，不再只靠入库时间猜测。
- 技术：GORM 增加 `year`；保存时可显式传入，缺省从标题正文提取或用当年；旧数据 `year=0` 读取时回退 `created_at`。
- 验证：`cd apps/api && go test ./...`；`pnpm --filter ./apps/web build`；`DB_AUTO_MIGRATE=true` 后重启后端，再入库一份教案并在画像按年查看。

## 2026-08-24 — 画像果实数量与成果库分类一致

- 业务：教师画像「专业荣誉 / 教研科研 / 特色实践」份数与成果库同类文档一致；奖状年份较早也不再从当前树视图消失。日常叶片仍按所选年份统计。
- 技术：画像与成果库用同一手机号拉个人文件夹；果实计数用同一套 `resolveArchivePlanCategory`。
- 验证：`pnpm --filter ./apps/web build`；硬刷新 `/profile` 后，成果库「专业荣誉」2 份时画像对应卡片应为 2。

## 2026-08-24 — 教师成果文档列表按三类筛选

- 业务：成果库「教师成果文档」增加特色实践 / 教研科研 / 专业荣誉筛选，卡片显示分类标签。
- 技术：列表优先合并本地 `archive_achievements`，旧文档仍从正文或标题推断。
- 验证：`pnpm --filter ./apps/web build`；打开 `/archive` 用分类芯片筛选证书类文档。

## 2026-08-24 — 成果解析按三类挂果并落库分类/年份

- 业务：成果解析智能体改为输出成长树三类（特色实践 / 教研科研 / 专业荣誉）和年份，上传成果库后写入本地表，画像树优先用库内分类而不是标题猜。
- 技术：扩展解析 JSON 与知识库正文元数据；新增 GORM 表 `archive_achievements`（`tree_category`、`year`）及 `/api/v1/archive-achievements`。生产库为 MySQL，GORM 同样可用于 PostgreSQL。
- 验证：`cd apps/api && go test ./...`；`pnpm --filter ./apps/web build`；把新系统提示词同步到平台智能体 14509；`DB_AUTO_MIGRATE=true` 后重启后端再上传一份成果。

## 2026-08-24 — 教师画像改为绿色专业成长树

- 业务：教师画像主界面按「专业成长树」呈现（叶片=周计划/活动方案，果实=实践/教研/荣誉）；成果库页不变；保留智能画像解读。
- 技术：React 重绘树与轨迹，数据接本人入库文档、成果库知识库与本地录入；配色为松针绿/苔绿/橄榄金，不做排名或绩效分。
- 验证：`pnpm --filter ./apps/web build`；登录后打开 `/profile` 确认树上数量与真实成果一致；`/archive` 仍为原成果库列表。

## 2026-08-19 — 教案/周计划知识库上传不再落到未分类

- 业务：知识库管理里上传 PDF/图片/Word 应进入「教案知识库管理」或「周计划管理」，而不是报「未在分类中确认到该文档」。
- 技术：业务库原文件改为 `file/upload` 取链接后走 `document/text` + `forceKind`（不再用会误入未分类的 `document/file`，也不挂成果解析 agent）；校验标题宽松匹配，必要时 `document/edit` 归入目标分类。
- 验证：`pnpm --filter ./apps/web build`；在活动方案知识库管理上传一份 PDF 或 PNG，应出现在该分类列表。
